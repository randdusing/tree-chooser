...
/***/ function(module, exports, __webpack_require__) {
	...
	var app = _angular2.default.module('app', ['treeChooser']).controller('treeChooserExample', function (treeChooserExample) {
	  ...
	  vm.model = vm.multiselect ? [] : null;
	  vm.treeData = treeChooserExample.getSampleData();
	...
	}).directive('treeChooserExample', function () {
	  return {
	    bindToController: true,
	    controller: 'treeChooserExample',
	    controllerAs: 'vm',
	    template: __webpack_require__(310)
	  };
	}).service('treeChooserExample', function () {
	  ...
	  vm.remove = function (item) {
	    vm.tree = _.cloneDeep(vm.tree);
	    _.remove(vm.tree, item);
	  };
	  ...
	  return {
	    ...
	    controller: 'tree',
	    controllerAs: 'vm',
	    require: '^treeChooserExample',
	    scope: {
	      tree: '=?'
	    },
	    template: '<li ng-repeat="item in vm.tree"><span>{{item.label}}</span><span ng-click="vm.remove(item)"> x</span><ul ng-if="item.children.length" tree="item.children"></ul></li>'
	  };
	});
